...
public class MainOld {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try (JedisPool pool = new JedisPool("192.168.1.103")) {
            ...
            Map<Integer, AtomicBroadcastReplica> replicas = setupReplicas(pool, Protocol.HONEY_BADGER);

            // Start proposer threads
            /*for (AtomicBroadcastReplica replica: replicas.values()) {
                Thread proposerThread = new Thread(() -> {
                    for (int i=0; i < 25; i++) {
                        replica.propose(String.format("r-%d-%d", replica.getId(), i)
    ...
    private static Map<Integer, AtomicBroadcastReplica> setupReplicas(JedisPool pool, Protocol protocol) {
        ...
        switch (protocol) {
            case HONEY_BADGER:
                encoder = new HoneyBadgerMessageEncoder(
                        new HoneyBadgerSubsetMessageEncoder(
            ...
            case DUMBO:
                encoder = new DumboMessageEncoder(
                        new DumboSubsetMessageEncoder(
                                new BrachaBroadcastMessageEncoder(),
                                new MoustefaouiBinaryAgreementMessageEncoder(),
            ...
            default:
                encoder = new AleaMessageEncoder(
                        new EchoVBroadcastMessageEncoder(), // FIXME use ECHO_V2
                        new MoustefaouiBinaryAgreementMessageEncoder()
                );
        }
        ...
        for (int replicaId: validatorSet.getAllIds()) {
            ...
            // Initialize atomic broadcast channel
            IAtomicBroadcast instance;
            switch (protocol) {
                case HONEY_BADGER:
                    Params hbParams = new Params(100L, encryptionSchedule, 8);
                    HoneyBadgerSubsetFactory hbSubset = new HoneyBadgerSubsetFactory(
                            replicaId,
                            networkInfo,
                            new BrachaBroadcastFactory(replicaId, networkInfo),
                            new MoustefaouiBinaryAgreementFactory(replicaId, networkInfo)
                    );
                    instance = new HoneyBadger(replicaId, networkInfo, hbParams, hbSubset);
                    break;

                case DUMBO:
                    pt.tecnico.ulisboa.hbbft.abc.dumbo.Params dumboParams = new pt.tecnico.ulisboa.hbbft.abc.dumbo.Params(100L, encryptionSchedule, 8, networkInfo.getF() + 1);
                    DumboSubsetFactory dumboSubset = new DumboSubsetFactory(
                            replicaId,
                            networkInfo,
                            new BrachaBroadcastFactory(replicaId, networkInfo),
                            new MoustefaouiBinaryAgreementFactory(replicaId, networkInfo)
                    );
                    instance = new Dumbo(replicaId, networkInfo, dumboParams, dumboSubset);
                    break;

                case ALEA_BFT:
                default:
                    Alea.Params aleaParams = new Alea.Params.Builder().batchSize(8).build();
                    instance = new Alea(replicaId, networkInfo, aleaParams);
            }
