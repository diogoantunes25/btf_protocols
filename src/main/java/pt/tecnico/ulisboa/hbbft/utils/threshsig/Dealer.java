...
public class Dealer {
    ...
    public static void main(final String[] args) {
        ...
        System.out.println(SigShare.combine(b, sigs1, K, L, gk.getModulus(), gk.getExponent()));

        final int[] S2 = { 2, 3, 4 };
        SigShare[] sigs2 = new SigShare[K];
        for (int i = 0; i < S2.length; i++)
            sigs2[i] = keys[S2[i]].sign(b);
        System.out.println(SigShare.combine(b, sigs2, K, L, gk.getModulus(), gk.getExponent()));

        System.out.println(
                SigShare.combine(b, sigs1, K, L, gk.getModulus(), gk.getExponent())
                        .equals(SigShare.combine(b, sigs2, K, L, gk.getModulus(), gk.getExponent())));
    }
}
